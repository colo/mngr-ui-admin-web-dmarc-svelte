...
module.exports = new Class({
  ...
  initialize: function (options) {
    ...
    if (!Array.isArray(this.clients)) { this.clients = [this.clients] }
  ...
  connect: function () {
    ...
    Array.each(this.clients, function (client, index) {
      if (!this.connected_clients || !this.connected_clients[index]) { // to prevent multiples ON_ERROR trying to reconnect
        if (!this.err_client_count[index]) { this.err_client_count[index] = 0 }
      ...
      Array.each(requests_by_type, function (app_req) {
        ...
        switch (type) {
          case 'once':
            ...
            // process.exit(1)
            if (this.options.suspended !== true) { this.dispatch(null, client, app, app_req, type) }
  ...
  _process_periodical: function (client, app, app_req, type) {
    ...
    if ((!window || window === undefined) && process && process.on) {
  ...
  _register_client: function (index) {
    ...
    if (!this.connected_clients) { this.connected_clients = [] }
    ...
    if (!this.connected_clients[index]) {
      ...
      if (this.err_client_count) { delete this.err_client_count[index] }

      if (this.err_clients) { delete this.err_clients[index] }
  ...
  _register_error: function (index, client) {
    ...
    if (!this.err_clients) { this.err_clients = [] }
    ...
    if (!this.err_clients[index]) {
      ...
      let reconnect_stop = function () { clearInterval(reconnect_timer) }
