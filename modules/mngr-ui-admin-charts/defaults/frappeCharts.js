    ...
    */
    transform: function (values, caller, chart, cb) {
      ...
      let isWindow = !!((caller === undefined || ( caller && caller.setInterval)))
			...
			if(caller === chart){//pre_process
				let data = { labels: [], datasets: [] }

				Array.each(values, function(row){
					debug('transform ROW', row)
					Array.each(row.value, function(val){
						if(Array.isArray(val) || !isNaN(val) ){
							debug('transform NOT object', val)
						}
						else{
							debug('transform object', val)
							data.labels.combine(Object.keys(val))
							data.datasets.push({ values: Object.values(val) })
						}
					})
				})

				debug('transform2', data)

				if(cb && typeof cb === 'function'){
					debug('transform3', data)
					cb(data)
				}
				else{
					return data
				...
				debug('transform data to tabular', values)

				let data = { labels: [], datasets: [] }
				let _return = []
				Array.each(values, function(row){
					debug('transform data to tabular ROW', row)
					let _data = Object.clone(data)
					if(!row.timestamp){
						_return.push(row)
					}
					else{
						Array.each(row.value, function(val){
							if(Array.isArray(val) || !isNaN(val) ){
								...
								debug('transform data to tabular object', val)
								_data.labels.combine(Object.keys(val))
								_data.datasets.push({ values: Object.values(val) })
							}
						})
						// row.value = [_data]
						_return.push([row.timestamp, _data])
						debug('transform data to tabular ROW2', row)
					}

				})

				if(cb && typeof cb === 'function'){
					debug('transform3', _return)
					cb(_return)
				}
				else{
					return _return
  ...
  pre_process: function (chart, name, stat) {
    ...
    if (!chart.options ||
      ...
      if (!chart.options.data.datasets) { chart.options.data.datasets = [] }
			...
			let data = chart.watch.transform(stat, chart, chart)

			if(data){
				if(data.labels) chart.options.data.labels = data.labels
				if(data.datasets) chart.options.data.datasets = data.datasets
			}
			// Array.each(stat, function(row){
			//
			// })
      // /**
      // * dynamic, like 'cpus', that is an Array (multiple cores) of Objects and we wanna watch a specific value
